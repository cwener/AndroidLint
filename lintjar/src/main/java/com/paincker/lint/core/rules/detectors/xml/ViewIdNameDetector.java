package com.paincker.lint.core.rules.detectors.xml;
...
/**
 * 定义代码检查规则
 * 这个是针对XML布局文件的控件id的前缀进行一个判断
 * 推荐使用规范好的ll,rl,fl,tl,btn,tv,iv,cb,pb等控件前缀。
 * </p>
...
public class ViewIdNameDetector extends ResourceXmlDetector {
    ...
    private static final String ISSUE_ID = "XTC_IdNameError";
    private static final String ISSUE_DESCRIPTION = "警告：控件命名应该符合规范";
    private static final String ISSUE_EXPLANATION = "警告：控件命名应该符合规范，这样才能更好的规范团队代码";
    ...
    String reportStrFormat = "警告：控件命名应该符合规范: {%s} 前缀应该为：{%s}";
