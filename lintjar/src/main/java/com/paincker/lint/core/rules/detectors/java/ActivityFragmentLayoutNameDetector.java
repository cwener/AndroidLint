package com.paincker.lint.core.rules.detectors.java;/*
...
public class ActivityFragmentLayoutNameDetector extends Detector
    ...
    private static final Class<? extends Detector> DETECTOR_CLASS = ActivityFragmentLayoutNameDetector.class;
    ...
    private void lintCheckMethodSetContentViewInActivity(@NonNull JavaContext context, @NonNull MethodInvocation node) {
        if (isSetContentViewOnThis_ForActivity(node)
            ...
            if (argument == null) {
                System.err.println("Custom Lint Error:Some thing went wrong in ActivityFragmentLayoutNameDetector.visitMethod:\n\targument of setContentView is null");
                return;
            }
...
//                System.out.println("lintCheckMethodInflateInFragment () 出现lint检测项，对应的责任人为： " + relativePersonName);
    ...
    /**
     * If user make this call "setContentView(R.layout.activity_main_origin)" without an explict instance, return true.
     * Else if "this.setContentView(R.layout.activity_main_origin)", then return true.
     * Else if "someobj.setContentView(R.layout.activity_main_origin)", return false.
     */
